...
def write_snapshot(path: Path, text: str) -> Path:
    if path.exists():
...
def save_config_snapshot(workspace: Path, cfg_path: Path, cfg: dict, args: argparse.Namespace, logger) -> None:
    snapshots = workspace / "config_snapshots"
    snapshots.mkdir(parents=True, exist_ok=True)
    source_copy = write_snapshot(snapshots / "config_original.yaml", cfg_path.read_text(encoding="utf-8"))
    effective_copy = write_snapshot(
        snapshots / "config_effective.yaml",
        yaml.safe_dump(cfg, sort_keys=False, allow_unicode=True),
    )
    args_copy = write_snapshot(
        snapshots / "run_args.json",
        json.dumps({
    ...
    logger.info("config snapshot saved: original=%s effective=%s args=%s", source_copy, effective_copy, args_copy)
...
def main() -> None:
    ...
    parser.add_argument("--config", default="detection_gligen_sdedit/configs/coco_mini.yaml")
    ...
    save_config_snapshot(workspace, cfg_path, cfg, args, logger)
    ...
    def run_mine(round_idx: int, current_weights: Path) -> Path:
        ...
        cmd = [
            sys.executable, "-m", "detection_gligen_sdedit.mine_failures",
        ...
        if cfg["mining"].get("min_failures_per_class") is not None:
            cmd += ["--min-failures-per-class", str(cfg["mining"]["min_failures_per_class"])]
        run(cmd, logger, workspace, f"round_{round_idx}_mine")
    ...
    def run_optimize(round_idx: int, current_weights: Path, failures: Path) -> Path:
        ...
        cmd = [
            sys.executable, "-m", "detection_gligen_sdedit.optimize_object_prompts",
            ...
            "--class-semantic-weight", str(cfg["prompt_optimization"].get("class_semantic_weight", 0.0)),
            "--class-semantic-model", str(cfg["prompt_optimization"].get("class_semantic_model", "openai/clip-vit-large-patch14")),
            "--save-steps", str(cfg["prompt_optimization"]["save_steps"]),
            "--log-every", str(cfg["prompt_optimization"]["log_every"]),
            "--seed", str(cfg["seed"]),
        ] + subset_args
        if cfg["prompt_optimization"].get("class_semantic_local_files_only", sd.get("local_files_only", True)):
            cmd.append("--class-semantic-local-files-only")
        if cfg["prompt_optimization"].get("include_class_name", False):
            cmd.append("--include-class-name")
        quality_filter = cfg["prompt_optimization"].get("instance_quality_filter", {})
        if bool(quality_filter.get("enabled", False)):
            cmd.append("--use-instance-quality-filter")
            if quality_filter.get("results"):
                cmd += ["--instance-quality-results", str(resolved(root, quality_filter["results"]))]
            if quality_filter.get("config"):
                cmd += ["--instance-quality-config", str(resolved(root, quality_filter["config"]))]
    ...
    def run_generate(round_idx: int, prompts: Path) -> tuple[Path, list[Path]]:
        ...
        generation_model = generation_cfg.get("model") or generation_cfg.get("gligen_model") or sd["model"]
        logger.info("round_%d GLIGEN SDEdit generate config: model=%s tokens_dir=%s output_dir=%s target_images=%d steps=%s strength=%s noise_timestep=%s",
                    round_idx, generation_model, prompts, synthetic, synthetic_images_per_round,
                    generation_cfg["num_inference_steps"], generation_cfg.get("strength"),
                    generation_cfg.get("noise_timestep"))
        cmd = [
            sys.executable, "-m", "detection_gligen_sdedit.generate_gligen_sdedit_examples",
            "--pretrained-model-name-or-path", str(generation_model),
            "--annotations", str(resolved(root, data["train_annotations"])),
            "--images", str(resolved(root, data["train_images"])),
            "--tokens-dir", str(prompts),
            "--output-dir", str(synthetic),
            "--max-images", str(synthetic_images_per_round),
            "--max-objects-per-image", str(generation_cfg["max_objects_per_image"]),
            "--strength", str(generation_cfg["strength"]),
            "--guidance-scale", str(generation_cfg["guidance_scale"]),
            "--num-inference-steps", str(generation_cfg["num_inference_steps"]),
            "--gligen-scheduled-sampling-beta", str(generation_cfg.get("gligen_scheduled_sampling_beta", 0.3)),
            "--width", str(generation_cfg.get("width", 512)),
            "--height", str(generation_cfg.get("height", 512)),
            "--global-prompt-source", str(generation_cfg.get("global_prompt_source", "template")),
            "--global-prompt-template", str(generation_cfg.get("global_prompt_template", "{phrases}, photo, highly detailed, photorealistic")),
            "--device", generation_cfg["device"],
            "--seed", str(cfg["seed"] + round_idx),
            "--clean-output",
        ] + subset_args
        if generation_cfg.get("variant") is not None:
            cmd += ["--variant", str(generation_cfg["variant"])]
        if generation_cfg.get("noise_timestep") is not None:
            cmd += ["--noise-timestep", str(generation_cfg["noise_timestep"])]
        if generation_cfg.get("negative_prompt") is not None:
            cmd += ["--negative-prompt", str(generation_cfg["negative_prompt"])]
        if bool(generation_cfg.get("save_visualization", False)):
            cmd.append("--save-visualization")
        if generation_cfg.get("include_class_name") is True:
            cmd.append("--include-class-name")
        elif generation_cfg.get("include_class_name") is False:
            cmd.append("--no-include-class-name")
        if bool(generation_cfg.get("enable_generation_filter", False)):
            cmd.append("--enable-generation-filter")
            cmd += ["--min-bbox-area-ratio", str(generation_cfg.get("min_bbox_area_ratio", 0.0))]
        if generation_cfg.get("local_files_only", sd.get("local_files_only", True)):
            cmd.append("--local-files-only")
        ...
        logger.info("round_%d GLIGEN SDEdit hard-example generation complete: synthetic_dir=%s selected=%d", round_idx, synthetic, len(selected))
        return synthetic, selected

    def run_finetune(run_output: Path, run_name: str, weights: Path, synthetic_dir: Path, epochs: int) -> Path:
        save_every_epochs = cfg["finetune"].get("save_every_epochs")
        logger.info("finetune config: output_dir=%s run_name=%s weights=%s synthetic_dir=%s epochs=%d batch_size=%s save_every_epochs=%s",
                    run_output, run_name, weights, synthetic_dir, epochs, cfg["finetune"]["batch_size"], save_every_epochs)
        cmd = [
            sys.executable, "-m", "detection_gligen_sdedit.finetune_yolov7",
            ...
            "--run-name", run_name,
        ] + train_eval_subset_args
        if save_every_epochs is not None:
            cmd += ["--save-every-epochs", str(save_every_epochs)]
        run(cmd, logger, workspace, run_name)
    ...
    def run_evaluate(round_idx: int, weights: Path) -> tuple[Path, dict]:
        ...
        run([
            sys.executable, "-m", "detection_gligen_sdedit.evaluate_yolov7",
            "--val-annotations", str(resolved(root, data["val_annotations"])),
            "--val-images", str(resolved(root, data["val_images"])),
