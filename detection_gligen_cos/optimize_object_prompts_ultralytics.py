...
from .prompt_gradient_alignment import (
    AlignmentRunningStatistics,
    build_layout_target,
    exact_alignment_cosine,
...
def load_instance_quality_rows(results_path: Path | None, config_path: Path | None, root: Path) -> tuple[Path, list[dict[str, Any]]]:
    ...
    if not results_path.exists():
        raise FileNotFoundError(
            ...
            "Run detection_gligen_cos.analyze_instance_quality first, or disable the filter."
        )
...
def main():
    parser = argparse.ArgumentParser(description="Depth-style object prompt optimization with Ultralytics detector feedback.")
    ...
    parser.add_argument("--detector-max-det", type=int, default=300)
    parser.add_argument(
        "--alignment-enabled",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Add exact detector-head gradient cosine alignment to Prompt optimization.",
    )
    parser.add_argument("--guide-images", default=None)
    parser.add_argument("--guide-labels", default=None)
    parser.add_argument("--alignment-cache-dir", default=None)
    parser.add_argument("--alignment-image-size", type=int, default=640)
    parser.add_argument(
        "--alignment-scaleup", action=argparse.BooleanOptionalAction, default=False
    )
    parser.add_argument("--guide-batch-size", type=int, default=8)
    parser.add_argument("--guide-workers", type=int, default=4)
    parser.add_argument("--max-guide-images", type=int, default=None)
    parser.add_argument(
        "--alignment-parameter-scope",
        choices=["detection_head"],
        default="detection_head",
    )
    parser.add_argument(
        "--alignment-implementation", choices=["exact"], default="exact"
    )
    parser.add_argument("--alignment-loss", choices=["hinge"], default="hinge")
    parser.add_argument("--alignment-weight", type=float, default=1.0)
    parser.add_argument("--alignment-margin", type=float, default=0.0)
    parser.add_argument("--alignment-warmup-steps", type=int, default=0)
    parser.add_argument("--alignment-epsilon", type=float, default=1e-12)
    parser.add_argument("--alignment-max-objects", type=int, default=20)
    parser.add_argument(
        "--alignment-enable-layout-filter", action="store_true"
    )
    parser.add_argument("--alignment-min-bbox-area-ratio", type=float, default=0.0)
    parser.add_argument(
    ...
    parser.add_argument("--initial-step", type=int, default=0,
                        help="Resume bookkeeping step; embeddings must be supplied with --resume-token.")
    ...
    if args.initial_step < 0 or args.initial_step > args.max_train_steps:
        parser.error("--initial-step must be between 0 and --max-train-steps")
    if args.initial_step and not args.resume_token:
        parser.error("--initial-step requires --resume-token")
    if args.alignment_enabled:
        if not args.guide_images or not args.guide_labels:
            parser.error("--alignment-enabled requires --guide-images and --guide-labels")
        if args.model_family != "yolo":
            parser.error("exact Prompt gradient alignment currently supports model-family=yolo only")
        if args.alignment_weight < 0:
            parser.error("--alignment-weight must be non-negative")
        if not -1.0 <= args.alignment_margin <= 1.0:
            parser.error("--alignment-margin must be in [-1,1]")
        if args.alignment_warmup_steps < 0:
            parser.error("--alignment-warmup-steps must be non-negative")
        if args.alignment_image_size <= 0 or args.guide_batch_size <= 0:
            parser.error("alignment image size and Guide batch size must be positive")
        if args.guide_workers < 0:
            parser.error("--guide-workers must be non-negative")
        if args.alignment_epsilon <= 0:
            parser.error("--alignment-epsilon must be positive")
        if args.alignment_max_objects <= 0:
            parser.error("--alignment-max-objects must be positive")
        if args.alignment_min_bbox_area_ratio < 0:
            parser.error("--alignment-min-bbox-area-ratio must be non-negative")
    ...
    logger.info(
        "dataset loaded: dataset_yaml=%s images=%d annotations=%d classes=%d class_mapping=%s",
        dataset.dataset_yaml,
        len(dataset.images),
        sum(len(v) for v in dataset.annotations.values()),
    ...
    failures = load_failures(
        args.failures,
        class_subset=class_subset,
        expected_class_names=dataset.class_names,
    )
    ...
    guide_cache_metadata: dict[str, Any] | None = None
    previous_alignment_statistics: dict[str, Any] | None = None
    previous_metadata_path = Path(args.output_dir) / "object_prompts.json"
    if args.initial_step and previous_metadata_path.is_file():
        ...
        previous_alignment = previous_metadata.get("gradient_alignment", {})
        previous_alignment_statistics = previous_alignment.get("statistics")
    alignment_statistics = AlignmentRunningStatistics(previous_alignment_statistics)
    detector_snapshot = None
    if args.alignment_enabled:
        cache_directory = Path(
            args.alignment_cache_dir
            or (Path(args.output_dir) / "gradient_alignment")
        )
        guide_gradients, guide_cache_metadata = load_or_compute_guide_gradients(
            detector=detector,
            image_directory=args.guide_images,
            label_directory=args.guide_labels,
            cache_directory=cache_directory,
            dataset_yaml=args.dataset_yaml,
            class_names=full_dataset.class_names,
            class_mapping=full_dataset.class_mapping,
            image_size=args.alignment_image_size,
            scaleup=args.alignment_scaleup,
            batch_size=args.guide_batch_size,
        ...
        if args.initial_step and previous_metadata_path.is_file():
            ...
            current_hash = guide_cache_metadata["metadata_hash"]
            if previous_hash is not None and previous_hash != current_hash:
                raise RuntimeError(
                    "Prompt checkpoint Guide metadata differs from the current alignment configuration; "
                    "start a new Prompt output directory"
                )
        detector_snapshot = [
            parameter.detach().cpu().clone()
            for parameter in detector.alignment_parameters()
        ]
        logger.info(
            "Prompt gradient alignment ready: implementation=%s parameters=%d numel=%d "
            "guide_images=%s cache_hit=%s guide_norm=%s metadata_hash=%s",
            args.alignment_implementation,
            len(detector.alignment_parameters()),
            sum(item["numel"] for item in detector.alignment_parameter_metadata()),
            guide_cache_metadata["metadata"]["guide_image_count"],
            guide_cache_metadata["cache_hit"],
            guide_cache_metadata["statistics"].get("gradient_norm"),
            guide_cache_metadata["metadata_hash"],
        )
    ...
    progress = tqdm(
        range(args.initial_step, args.max_train_steps),
        total=args.max_train_steps,
        initial=args.initial_step,
        disable=not accelerator.is_local_main_process,
    )
    logger.info("training start: steps=%d lr=%s scheduler=%s strength=%s denoise_steps=%d semantic_weight=%s detector_weight=%s class_semantic_weight=%s alignment_enabled=%s alignment_weight=%s alignment_margin=%s alignment_warmup_steps=%s",
                ...
                args.alignment_enabled, args.alignment_weight, args.alignment_margin,
                args.alignment_warmup_steps)
    logger.info("detector loss target policy: full composite image input; single failure-instance target only")
    if args.alignment_enabled:
        logger.info(
            "alignment target policy: full composite image input; failure-first full layout; "
            "max_objects=%d filter=%s min_bbox_area_ratio=%s letterbox=%d scaleup=%s",
            args.alignment_max_objects,
            args.alignment_enable_layout_filter,
            args.alignment_min_bbox_area_ratio,
            args.alignment_image_size,
            args.alignment_scaleup,
        )
    ...
    for global_step in progress:
        ...
        layout_target = None
        if args.alignment_enabled:
            layout_target = build_layout_target(
                full_dataset,
                sample,
                accelerator.device,
                max_objects=args.alignment_max_objects,
                enable_filter=args.alignment_enable_layout_filter,
                min_bbox_area_ratio=args.alignment_min_bbox_area_ratio,
            )
        ...
        with accelerator.accumulate(text_encoder):
            ...
            detector_loss_result = detector.differentiable_loss(detector_input, detector_target)
            detector_loss = detector_loss_result.total
            ...
            cosine = torch.zeros((), device=accelerator.device)
            cosine_loss = torch.zeros((), device=accelerator.device)
            cosine_weight_effective = 0.0
            cosine_dot = torch.zeros((), device=accelerator.device)
            generated_gradient_norm = torch.zeros((), device=accelerator.device)
            guide_gradient_norm = torch.zeros((), device=accelerator.device)
            alignment_detector_loss = torch.zeros((), device=accelerator.device)
            if args.alignment_enabled:
                if guide_gradients is None or layout_target is None:
                    raise RuntimeError("Gradient alignment was enabled but its Guide/target is unavailable")
                alignment_result, cosine_result = exact_alignment_cosine(
                    detector=detector,
                    image=detector_input,
                    targets=layout_target,
                    guide_gradients=guide_gradients,
                    image_size=args.alignment_image_size,
                    scaleup=args.alignment_scaleup,
                    epsilon=args.alignment_epsilon,
                )
                alignment_detector_loss = alignment_result.total
                cosine = cosine_result.cosine
                cosine_dot = cosine_result.dot
                generated_gradient_norm = cosine_result.generated_norm
                guide_gradient_norm = cosine_result.guide_norm
                cosine_loss = F.relu(
                    torch.as_tensor(
                ...
                warmup_ratio = (
                    ...
                    if args.alignment_warmup_steps == 0
                        ...
                        1.0
                        if args.alignment_warmup_steps == 1
                        else min(global_step / (args.alignment_warmup_steps - 1), 1.0)
                    )
                )
                cosine_weight_effective = args.alignment_weight * warmup_ratio
                alignment_statistics.update(group, float(cosine.detach()))
            
            loss = (
                args.semantic_weight * semantic
                + args.class_semantic_weight * class_semantic
                - args.detector_weight * detector_loss
                + cosine_weight_effective * cosine_loss
            )
        ...
        if global_step == 0 or (global_step + 1) % args.log_every == 0:
            progress.set_postfix(loss=float(loss.detach()), semantic=float(semantic.detach()), class_sem=float(class_semantic.detach()), det=float(detector_loss.detach()), cos=float(cosine.detach()), group=group)
            logger.info("step=%d/%d group=%s loss=%.6f semantic=%.6f class_semantic=%.6f det=%.6f alignment_det=%.6f cos=%.6f cos_loss=%.6f cos_weight=%.6f generated_grad_norm=%.6f guide_grad_norm=%.6f dot=%.6f active_targets=%d layout_targets=%d lr=%.6g",
                        global_step + 1, args.max_train_steps, group, float(loss), float(semantic),
                        float(class_semantic), float(detector_loss), float(alignment_detector_loss),
                        float(cosine), float(cosine_loss), cosine_weight_effective,
                        float(generated_gradient_norm), float(guide_gradient_norm), float(cosine_dot),
                        int(target.shape[0]), int(layout_target.shape[0]) if layout_target is not None else 0,
        ...
        if accelerator.is_main_process and ((global_step + 1) % args.save_steps == 0 or global_step + 1 == args.max_train_steps):
            if detector_snapshot is not None:
                for index, (before, parameter) in enumerate(
                    zip(detector_snapshot, detector.alignment_parameters())
                ):
            ...
            metadata = {"prompt_scope": args.prompt_scope, "num_new_tokens": args.num_new_tokens,
                        ...
                        "gradient_alignment": {
                            "enabled": bool(args.alignment_enabled),
                            "implementation": args.alignment_implementation if args.alignment_enabled else None,
                            "parameter_scope": args.alignment_parameter_scope if args.alignment_enabled else None,
                            "loss": args.alignment_loss if args.alignment_enabled else None,
                            "weight": args.alignment_weight if args.alignment_enabled else 0.0,
                            "margin": args.alignment_margin if args.alignment_enabled else None,
                            "warmup_steps": args.alignment_warmup_steps if args.alignment_enabled else 0,
                            "epsilon": args.alignment_epsilon if args.alignment_enabled else None,
                            "target_scope": "full_layout" if args.alignment_enabled else None,
                            "image_size": args.alignment_image_size if args.alignment_enabled else None,
                            "scaleup": args.alignment_scaleup if args.alignment_enabled else None,
                            "guide_cache": guide_cache_metadata,
                            "guide_metadata_hash": guide_cache_metadata["metadata_hash"] if guide_cache_metadata else None,
                            "parameters": detector.alignment_parameter_metadata() if args.alignment_enabled else [],
                            "statistics": alignment_statistics.to_dict(),
                        }}
